...
function Items:AddButton(Label, Description, Style, Actions, Submenu)
	...
	if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
		...
		if not (Style.IsDisabled) then
			...
			if Style.Emoji then
				Graphics.Text(Style.Emoji, CurrentMenu.X + 8 + LeftBadgeOffset, CurrentMenu.Y + 5 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.28, Active and 0 or 245, Active and 0 or 245, Active and 0 or 245, 255)
			end
		else
			...
			if Style.Emoji then
				Graphics.Text(Style.Emoji, CurrentMenu.X + 8 + LeftBadgeOffset, CurrentMenu.Y + 5 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.28, 163, 159, 148, 255)
			end
		end
...
function Items:AddSeparator(Label)
	...
	if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
		local Active = CurrentMenu.Index == Option;
		if (Label ~= nil) then
			Graphics.Text(Label, CurrentMenu.X + 0 + (CurrentMenu.WidthOffset * 1.85 ~= 0 and CurrentMenu.WidthOffset * 1.85 or 200), CurrentMenu.Y + 0 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.33, 245, 245, 245, 125, 1)
		end
		RageUI.ItemOffset = RageUI.ItemOffset + 38
		if (Active) then
		...
		if (not Style.IsDisabled) then
			if (Active) then
				if Label ~= "" then
					...
					Graphics.Text(ListText, CurrentMenu.X + 295 + LeftBadgeOffset, CurrentMenu.Y + 3 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.33, 0, 0, 0, 255, 1)
				end
			else
				if Label ~= "" then
					Graphics.Text(Label, CurrentMenu.X + 8 + LeftBadgeOffset, CurrentMenu.Y + 3 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.33, 245, 245, 245, 255)
					...
					Graphics.Text(ListText, CurrentMenu.X + 295 + LeftBadgeOffset, CurrentMenu.Y + 3 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.33, 245, 245, 245, 255, 1)
				end
			end
		else
			if Label ~= "" then
				...
				Graphics.Text(ListText, CurrentMenu.X + 295 + LeftBadgeOffset, CurrentMenu.Y + 3 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, 0.33, 163, 159, 148, 255, 1)
			end
		end
